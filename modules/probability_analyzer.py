...
from .utils import CacheManager, hash_params, hash_dataframe, calculate_annualized_return


class ProbabilityAnalyzer:
    def __init__(self, df: pd.DataFrame, cache_manager: Optional[CacheManager] = None):
        self.df = df
        self.cache_manager = cache_manager
        self._trading_dates_cache: Optional[set] = None
    
    def _get_trading_dates(self) -> set:
        if self._trading_dates_cache is None:
            self._trading_dates_cache = set(self.df['日期'].dt.date)
        return self._trading_dates_cache
    
    def _get_cache_key(self, prefix: str, **params) -> str:
        params_hash = hash_params(params)
        df_hash = hash_dataframe(self.df)
        return f"{prefix}_{df_hash}_{params_hash}"
    
    def _apply_sampling(self, start_dates: List[date], sampling: str) -> List[date]:
        if sampling == 'monthly':
            sampled_dates = []
            current_year = None
            current_month = None
    ...
    def get_all_possible_start_dates(
        self, 
        ...
        df_filtered = self.df[
            (self.df['日期'].dt.date >= analysis_start_date) & 
            (self.df['日期'].dt.date <= analysis_end_date)
        ]
        ...
        max_data_date = self.df['日期'].max().date()
        
        valid_start_dates = []
        for d in trading_dates:
            end_date = d + timedelta(days=duration_days)
            if end_date <= max_data_date:
                valid_start_dates.append(d)
    ...
    def run_single_backtest(
        ...
        while actual_end_date not in trading_dates and actual_end_date <= self.df['日期'].max().date():
            actual_end_date += timedelta(days=1)
        
        investment_dates = get_investment_dates(self.df, start_date, actual_end_date, freq_type, freq_param)
        
        if len(investment_dates) == 0:
            return None
        
        results_df, total_shares_ideal, total_investment, total_purchase_fee = run_backtest_calculation(
            self.df, investment_dates, amount, realistic_params
        )
        
        if len(results_df) == 0:
            return None
        
        dates = self.df['日期'].dt.date
        end_row = self.df[dates == actual_end_date]
        if len(end_row) == 0:
            end_row = self.df[dates <= actual_end_date].tail(1)
        ...
        ideal_annualized = calculate_annualized_return(ideal_total_return, actual_years)
        ...
        if realistic_params:
            ...
            real_annualized = calculate_annualized_return(real_total_return, actual_years)
    ...
    def run_single_smart_backtest(
        ...
        results_df, total_shares_ideal, total_investment, total_purchase_fee, total_deposited, cash_balance, total_from_sale, total_redemption_fee = run_smart_backtest_calculation(
            self.df, investment_dates, base_amount, strategy_config, realistic_params, use_cash_flow
        )
        ...
        if use_cash_flow:
            ...
            annualized_with_cash = calculate_annualized_return(total_return_with_cash, actual_years)
        else:
    ...
    def _run_parallel_backtests(
        ...
        if max_workers > 1 and total > 4:
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                future_to_date = {
                    executor.submit(wrapper_func, args): start_date
                    for start_date in start_dates
                    for args in [(self.df, start_date)]
                }
            ...
            for i, start_date in enumerate(start_dates):
                result = backtest_func(start_date)
                if result is not None:
                    results.append(result)
                
                if progress_callback:
                    elapsed = time.time() - start_time
                    progress_callback(i + 1, total, elapsed)
    ...
    def run_probability_analysis(
        ...
        if self.cache_manager:
            ...
            if cached is not None:
                return cached['results'], cached['elapsed_time']
        
        start_dates = self.get_all_possible_start_dates(
            analysis_start_date, analysis_end_date, investment_duration_years
        )
        
        start_dates = self._apply_sampling(start_dates, sampling)
        ...
        if max_workers > 1 and total > 4:
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                future_to_date = {
                    executor.submit(
                        ...
                        (self.df, start_date, investment_duration_years, freq_type, freq_param, amount, realistic_params)
                    ): start_date
                    for start_date in start_dates
                }
            ...
            for i, start_date in enumerate(start_dates):
                result = self.run_single_backtest(
                    start_date, investment_duration_years, freq_type, freq_param, amount, realistic_params
                )
                if result is not None:
                    results.append(result)
        ...
        if cache_key and self.cache_manager:
            self.cache_manager.set(cache_key, {'results': results, 'elapsed_time': elapsed_time})
    ...
    def calculate_statistics(
        ...
        params_hash = hash_params(realistic_params) if realistic_params else "None"
    ...
    def _hash_results(self, results: List[Dict[str, Any]]) -> str:
        ...
        return self._hashsum(hash_parts)
    
    def _hashsum(self, parts: List[str]) -> str:
        import hashlib
        return hashlib.md5('|'.join(parts).encode()).hexdigest()
    
    def run_smart_probability_analysis(
        self,
        analysis_start_date: date,
        analysis_end_date: date,
        investment_duration_years: float,
        ...
        if max_workers > 1 and total > 4:
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                future_to_date = {
                    executor.submit(
                        _run_single_smart_backtest_wrapper,
                        (self.df, start_date, investment_duration_years, freq_type, freq_param, 
            ...
            for i, start_date in enumerate(start_dates):
                result = self.run_single_smart_backtest(
                    start_date, investment_duration_years, freq_type, freq_param,
                    base_amount, strategy_config, realistic_params, use_cash_flow
                )
                if result is not None:
                    results.append(result)
    ...
    def run_comparison_probability_analysis(
        self,
        analysis_start_date: date,
        analysis_end_date: date,
        investment_duration_years: float,
        ...
        if self.cache_manager:
            cache_key = self._get_cache_key(
                'comparison_probability_analysis',
                analysis_start_date=str(analysis_start_date),
                analysis_end_date=str(analysis_end_date),
                investment_duration_years=investment_duration_years,
                freq_type=freq_type,
            ...
            cached = self.cache_manager.get(cache_key)
            if cached is not None:
                return cached['fixed_results'], cached['smart_results'], cached['elapsed_time']
        ...
        if max_workers > 1 and total > 4:
            with ProcessPoolExecutor(max_workers=max_workers) as executor:
                future_to_date = {
                    executor.submit(
                        _run_comparison_backtest_wrapper,
                        (self.df, start_date, investment_duration_years, freq_type, freq_param,
                         base_amount, strategy_config, realistic_params, use_cash_flow)
                    ): start_date
                    for start_date in start_dates
            ...
            for i, start_date in enumerate(start_dates):
                fixed_result = self.run_single_backtest(
                    start_date, investment_duration_years, freq_type, freq_param, base_amount, realistic_params
                )
                smart_result = self.run_single_smart_backtest(
                    start_date, investment_duration_years, freq_type, freq_param,
                    base_amount, strategy_config, realistic_params, use_cash_flow
                )
    ...
    def calculate_comparison_statistics(
        ...
        fixed_hash = self._hash_results(fixed_results)
        smart_hash = self._hash_results(smart_results)
        params_hash = hash_params(realistic_params) if realistic_params else "None"
...
def get_all_possible_start_dates(df, analysis_start_date, analysis_end_date, investment_duration_years):
    analyzer = ProbabilityAnalyzer(df)
    return analyzer.get_all_possible_start_dates(analysis_start_date, analysis_end_date, investment_duration_years)


def run_single_backtest(df, start_date, investment_duration_years, freq_type, freq_param, amount, realistic_params=None):
    analyzer = ProbabilityAnalyzer(df)
    return analyzer.run_single_backtest(start_date, investment_duration_years, freq_type, freq_param, amount, realistic_params)
...
def run_probability_analysis(df, analysis_start_date, analysis_end_date, investment_duration_years,
    ...
    def wrapped_callback(completed, total, elapsed):
        if progress_callback:
            progress_callback(completed, total)
    
    return analyzer.run_probability_analysis(
        analysis_start_date, analysis_end_date, investment_duration_years,
        freq_type, freq_param, amount, realistic_params, sampling, wrapped_callback if progress_callback else None
    )
...
def run_single_smart_backtest(df, start_date, investment_duration_years, freq_type, freq_param, 
    ...
    analyzer = ProbabilityAnalyzer(df)
    return analyzer.run_single_smart_backtest(
        start_date, investment_duration_years, freq_type, freq_param,
        base_amount, strategy_config, realistic_params, use_cash_flow
    )
...
def run_smart_probability_analysis(df, analysis_start_date, analysis_end_date, investment_duration_years,
    ...
    return analyzer.run_smart_probability_analysis(
        analysis_start_date, analysis_end_date, investment_duration_years,
        freq_type, freq_param, base_amount, strategy_config, realistic_params,
        sampling, wrapped_callback if progress_callback else None, use_cash_flow
    )
...
def run_comparison_probability_analysis(df, analysis_start_date, analysis_end_date, investment_duration_years,
    ...
    return analyzer.run_comparison_probability_analysis(
        analysis_start_date, analysis_end_date, investment_duration_years,
        freq_type, freq_param, base_amount, strategy_config, realistic_params,
        sampling, wrapped_callback if progress_callback else None, use_cash_flow
    )
